...
AdsMap.prototype.build_map = function () {
	...
	if (self.optOptions.showScreenShotButton) {
		var snapShootButton = document.createElement('button');
		snapShootButton.innerHTML = 'Take Screenshot';
		snapShootButton.className = 'btn btn-default btn-sm';
		snapShootButton.onclick = function () {
			if (typeof  self.optOptions.showScreenShotButton == 'function') {
				return self.optOptions.showScreenShotButton();
			} else {
				self.snapshot({
					onrendered: function (canvas) {
						//document.body.appendChild( canvas );
						var img = canvas.toDataURL("image/png");
						//var $image = AdsMap.Campaign.image(self, img, 'map', '');
						var image = '<img src="' + img + '" /><input type="hidden" name="screenshots[]" value="' + img + '" />';

						$('#map_images').append(image);

						//$html.find('#map_images').append($image);
		...
		this.map.controls[google.maps.ControlPosition.TOP_LEFT].push(snapShootButton);
	}
