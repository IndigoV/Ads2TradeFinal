...
class rfp extends CI_Model {
    ...
    function getRfpForId($id) {

        $query = $this->db->query("SELECT 
        campaign.cam_id,
        campaign.cam_requested_start_date,
        campaign.cam_requested_end_date,
        campaign.cam_title,
        campaign.cam_budget,
        campaign.cam_requested_response_date,
        campaign.cam_description,
        campaign.adv_id,
        campaign.cas_id,
        campaign.cam_number,
        campaign.cab_id,
        campaign.cam_order_number,
        campaign.cur_id,
        campaign.cam_risk_limit,
        campaign.cam_current_risk,
        campaign.cst_id,
        campaign.cam_partial_availability_status,
        campaign.cam_creation_date,
        campaign.cam_status,
        campaign.processed,
        campaign.pro_status_id,
        proposal_status.prs_description
        FROM campaign 
        INNER JOIN rfp_for_submission ON rfp_for_submission.campaign_id = campaign.cam_id 
        INNER JOIN proposal_status ON campaign.pro_status_id = proposal_status.pro_status_id
        WHERE campaign.cam_status = 0 AND campaign.cam_id = " . $id . " AND campaign.cam_requested_start_date <> 0000-00-00 AND rfp_for_submission.complete = 0 ORDER BY campaign.cam_id DESC");
    ...
    function getRfpForIdForComplete($id) {
        $sql = "SELECT * FROM campaign INNER JOIN rfp_for_submission ON rfp_for_submission.campaign_id = campaign.cam_id WHERE campaign.cam_status = 0 AND campaign.cam_id = " . $id . " AND campaign.cam_requested_start_date <> 0000-00-00 AND (rfp_for_submission.media_owner_id = ". $this->session->userdata('user_id') ." OR rfp_for_submission.operator_id = ". $this->session->userdata('user_id') .") ORDER BY campaign.cam_id DESC";
        $query = $this->db->query($sql);
        return $query->result()[0];
    }

    function getAllScreenShotsForCampaign($rfpID) {
        $query = $this->db->query("SELECT * FROM rfp_campaign_screenshot WHERE cam_id = '". $rfpID ."'");
//        echo '<pre>';
//        var_dump($query->result());
        $array = array();
        $i = 0;
        foreach($query->result() as $image) {
            $array[$i] = $image->image_name;
            $i++;
        }
//        echo '</pre>';
        return $array;
    ...
    function getLocations() {
        if (isset($_GET['id'])){
            $query = $this->db->query("SELECT * FROM rfp_transactions WHERE cam_id = '". $_GET['id'] ."'");
//            echo '<pre>';
//            var_dump($query->result());
//            echo '</pre>';
            return $query->result();
    ...
    function getRfp () {
      ...
      //  $this->db->where('rfp_id', $_GET['id']);
       ...
       // $select_query = $this->db->get('rfp');
        ...
        $this->db->where_not_in('cam_requested_start_date', '0000-00-00');
        $this->db->order_by('cam_id', 'desc');
        $this->db->join('proposal_status', 'proposal_status.pro_status_id = campaign.pro_status_id');
    ...
    function mo_getRfp () {
                ...
                $this->db->where('proposal.pro_status_id', NULL);
    ...
    function add_media_owner() {
        ...
        function getRfpProposal($campaignId=null) {
//            $query = $this->db->query('SELECT * FROM rfp_for_submission_assets_selected WHERE to_id = ' . $this->session->userdata('user_id'));
            $sql = "SELECT *
                    FROM rfp_for_submission_assets_selected
                    INNER JOIN rfp_for_submission
                    ON rfp_for_submission_assets_selected.rfp_for_submission_id = rfp_for_submission.rfp_for_submission_id
                    WHERE to_id = " . $this->session->userdata('user_id') . " AND (rfp_for_submission.proccessed_operator = '0' OR rfp_for_submission.process = '1')";

            if( $campaignId != null ) {
                $sql .= " AND rfp_for_submission.campaign_id=". $campaignId;
            }
     ...
     #record upload data
    function uploadDetails($uploaded_data)
    {
      $this->db->insert('uploaded_documents', $uploaded_data);
