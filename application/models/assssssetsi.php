...
//for pintos            
	...
	function getAsset() {
		...
		$select_query = $this->db->join('measurement_unit', 'measurement_unit.meu_id = asset.exposure_time_meu_id', 'left outer');
...
//var_dump($select_query);exit;
	...
	function getAssetFaceCount($user_id){
		//Also return mall_id (if any)
		$select_query = "
			SELECT
				uploaded_documents.upd_id, uploaded_documents.upd_url,
				uploaded_documents.upd_file_name, face.fac_description,
				face.fac_number, face.fac_reference_number
			FROM
				uploaded_documents
			LEFT JOIN face ON uploaded_documents.fac_id = face.fac_id
			WHERE
				uploaded_documents.ass_id = '$asset_id'";				
		$query = $this->db->query($select_query);
		if($query->num_rows > 0){
			return $query->num_rows;
		} else {select_queryselect_queryselect_query
		    return 1;
		}
	}
	...
	function insertFile($fpath1,$fsize, $fname, $ftype, $fac_id, $ass_id){
        ...
        $this->db->query("INSERT INTO `uploaded_documents` 
        	(`upd_url`, `upd_filesize`, `upd_file_name`, `upd_file_type`, `fac_id`, `ass_id`) 
        	VALUES 1
        	('$fpath1', '$fsize', '$fname', '$ftype', '$fac_id', '$ass_id')");
	}
