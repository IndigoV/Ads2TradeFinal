...
class Layouts
{
	...
	public function view($view_name, $layouts = array(), $params = array(), $default = true)
	{
		...
		{
			$view_subfolder = array('auction','campaig'); //All modules that must use the custom header and footer
			//$view_subfolder = "auction,campaig";
			$this_view_path = strtolower(substr($view_name, 0, 7));
			//echo $this_view_path. ":". $view_subfolder. ":".in_array($this_view_path, $view_subfolder); 
			if($this_view_path == 'auction' || in_array($this_view_path, $view_subfolder) ){
				//Load Auction header and footer instead of the default ones 
				...
				// render default header
				$this->CI->load->view("auction/header", $header_params);
				
				// render content
				$this->CI->load->view($view_name, $params);
				
				// render footer
				$this->CI->load->view("auction/footer");
