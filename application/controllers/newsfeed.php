...
class Newsfeed extends CI_Controller {
    ...
    public function index() {
        ...
        //Set default suspended flag
		$suspended = $this->input->get_post('suspended');
		if($suspended == ''){
			//$_POST['suspended'] = 0;
		}
    ...
    */
    ...
    public function create() {
        ...
        if($this->input->post("create-news")) {
            ...
            if( $this->form_validation->run() == false && false) {
                ...
                exit("Invalid Input! Please check your news article and make the necessary changes.");
            } else {
    ...
    public function edit($id) {
        ...
        if($this->input->post("update-news")) {
            ...
            if( $this->form_validation->run() == false && false) {
				...
				//	TODO: No need to work on a new news object since we want to update the current newsfeed we have just obtained
                ...
                $newsfeed->suspended = $this->input->post("suspended");				
                $newsfeed->news_date = $this->input->post("news_date"); //desired format: date('Y-m-d h:m:s');
				//TODO: Why update this object twice?????
                //$this->newsfeed->saveNewsfeed($newsfeed);
    ...
    public function view($id) {
        ...
        $this->layouts->view("newsfeeds/view", null, $data);
    }
