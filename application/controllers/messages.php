...
class Messages extends CI_Controller
{
	...
	public function inbox()
	{
		...
		if(isset($write) && $write == 1){
			...
			//*** Get message being replied or fowarded (if any)
			
			if($msgid > 0){
				$data['message'] = $this->events->getById($msgid);
				//mark message as read (if not already)
				if($data['message']->isread == 0){
					$message_read = $this->events->markMessageAsRead($msgid, 1);
				}
	...
	public function send()
	{
		...
		*/
		...
		$message->reply_of = $this->input->post('reply_of', TRUE);
